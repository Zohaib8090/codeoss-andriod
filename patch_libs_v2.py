...
for filename in files:
    if filename in RENAME_MAP:
        ...
        # Handle cases like libicui18n.so.78.3
        base = filename
...
for filename in new_files:
    ...
    try:
        ...
        print(f" Warning: Could not set SONAME for {filename}: {e}")

    # Update DT_NEEDED
    for old_name, new_name in RENAME_MAP.items():
        try:
            # We don't check if it's needed first, patchelf will just do nothing if not found
